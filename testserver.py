...
try:
    with open("serversettings.json") as f:
        fa=json.load(f)
        api_ip = fa["Http"]["Host"]
        api_port = int(fa["Http"]["Port"])
        listening_ip = fa["HttpPost"]["Host"]
        listening_port = int(fa["HttpPost"]["Port"])
except:
    api_ip = "127.0.0.1"
    api_port = 5800
...
def index(node):
    ...
    if node == "send_private_msg":
        ...
        if message_type == "private" and user_id is not None:
            req = requests.get(f"http://{api_ip}:{api_port}/send_private_msg?user_id={user_id}&message={message}")
            return req.json(), req.status_code
        elif message_type == "group" and group_id is not None:
            req = requests.get(f"http://{api_ip}:{api_port}/send_group_msg?group_id={group_id}&message={message}")
            return req.json(), req.status_code
        else:
            return "ok", 400
...
if __name__ == "__main__":
    ...
    while True:
        ...
        url = f"http://{listening_ip}:{listening_port}"
